...
class DataManagementService(private val context: Context) {
    ...
    companion object {
        ...
        enum class SessionStatus {
            ACTIVE, COMPLETED, ARCHIVED, EXPORTED, ERROR
        }
    }
    ...
    suspend fun queueFilesForUpload(sessionId: String): List<String> {
        ...
        for (fileMetadata in session.files) {
            ...
            try {
                val fileType = when (fileMetadata.fileType) {
                ...
                val jobId = uploadService.queueUpload(
    ...
    suspend fun performCleanup(maxAgeMs: Long = 7 * 24 * 60 * 60 * 1000L) { // 7 days default
        ...
        // Clean old sessions
        val sessionsToRemove = activeSessions.values.filter { session ->
     ...
     */
    private fun exportSessionAsJSON(session: SessionData, exportFile: File, includeFiles: Boolean) {
        val json = JSONObject().apply {
            put("session_id", session.sessionId)
            put("device_id", session.deviceId)
            put("start_time", session.startTime)
    ...
    private fun exportSessionAsCSV(session: SessionData, exportFile: File) {
        ...
        // Data rows
        session.files.forEach { file ->
            csvContent.appendLine("${file.fileId},${file.fileName},${file.fileType},${file.sizeBytes},${file.checksum},${file.timestamp},${file.mimeType}")
        }
     ...
     */
    private fun exportSessionAsZIP(session: SessionData, exportFile: File, includeFiles: Boolean) {
     ...
     */
    private fun generateFileId(sessionId: String, deviceId: String, fileName: String): String {
