...
class NetworkController(private val context: Context) {
    ...
    interface NetworkControllerListener {
        fun onStartRecordingCommand(sessionId: String, modalities: List<String>, options: Map<String, Any>)
     ...
     */
    private suspend fun handleClientMessages(connection: ClientConnection) = withContext(Dispatchers.IO) {
        try {
        ...
        try {
            ...
            when (command) {
                ...
                "get_status" -> handleGetStatusCommand(connection, json)
                    ...
                    sendResponse(connection, createErrorResponse("unknown_command", "Unknown command: $command"))
                }
            }
            
        } catch (e: Exception) {
            Log.e(TAG, "Error parsing command", e)
            sendResponse(connection, createErrorResponse("parse_error", "Failed to parse command: ${e.message}"))
        }
    }
    ...
    private suspend fun handleStartRecordingCommand(connection: ClientConnection, json: JSONObject) {
        try {
            ...
            // Send acknowledgment
            sendResponse(connection, createResponse("recording_started", "Recording session started", mapOf(
            ...
            sendResponse(connection, createErrorResponse("start_recording_error", e.message ?: "Unknown error"))
        }
    }
    ...
    private suspend fun handleStopRecordingCommand(connection: ClientConnection, json: JSONObject) {
        try {
            ...
            // Send acknowledgment
            sendResponse(connection, createResponse("recording_stopped", "Recording session stopped"))
            
        } catch (e: Exception) {
            Log.e(TAG, "Error handling stop_recording command", e)
            sendResponse(connection, createErrorResponse("stop_recording_error", e.message ?: "Unknown error"))
        }
    }
     ...
     */
    private fun createResponse(status: String, message: String, data: Map<String, Any> = emptyMap()): String {
        val json = JSONObject()
