...
class TimeManager(
    ...
    */
    suspend fun synchronizeWithPC(pcControllerAddress: String, port: Int = 8082): Boolean {
    ...
    Log.d(TAG, "Sync round ${attempt + 1}: offset=${syncResult.clockOffsetNs}ns, RTT=${syncResult.roundTripTimeNs / 1_000_000}ms")
    }
    ...
    private suspend fun performTimeSyncRound(pcAddress: String, port: Int): TimeSyncResult? {
    ...
    driftMonitoringJob = syncScope.launch {
    ...
    */
    ...
    val timeSinceSync = if (lastSyncTimestamp.get() > 0) {
    ...
    */
    fun calculateTimeDifferenceNs(timestamp1: Long, timestamp2: Long): Long {
    ...
    */
    fun areTimestampsSynchronized(timestamp1: Long, timestamp2: Long, toleranceMs: Double = SYNC_QUALITY_THRESHOLD_MS): Boolean {
