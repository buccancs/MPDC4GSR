...
class SessionDirectoryManagerTest {
    ...
    fun setup() {
        ...
        `when`(mockPackageManager.getPackageInfo("com.topdon.tc001.test", 0)).thenReturn(mockPackageInfo)
    ...
    fun testGenerateSessionId() {
        ...
        assertTrue(parts[3].matches(Regex("[a-zA-Z0-9]+")), "Fourth part should be alphanumeric device model")
    ...
    fun testCleanupFailedSessions() {
        ...
        File(successSessionDir.rgbDir, "test_data.txt").writeText("Some test data that makes this session valid")
        ...
        assertTrue(cleanedSessions.contains(failedSessionId), "Cleaned sessions should include failed session")
        assertFalse(cleanedSessions.contains(successSessionId), "Cleaned sessions should not include successful session")
    }
    @Test
    fun testGetStandardFilePath() {
        val sessionId = "20231201_120000_123_TestDevice_abcd1234"
        val sessionDir = sessionDirectoryManager.createSessionDirectory(sessionId)
        val rgbFile = sessionDirectoryManager.getStandardFilePath(sessionDir, "RGB", "test_video.mp4")
        assertEquals(sessionDir.rgbDir, rgbFile.parentFile)
        assertEquals("test_video.mp4", rgbFile.name)
        val thermalFile = sessionDirectoryManager.getStandardFilePath(sessionDir, "thermal", "thermal_data.csv")
        assertEquals(sessionDir.thermalDir, thermalFile.parentFile)
        assertEquals("thermal_data.csv", thermalFile.name)
        val shimmerFile = sessionDirectoryManager.getStandardFilePath(sessionDir, "Shimmer3", "gsr_data.csv")
        assertEquals(sessionDir.shimmerDir, shimmerFile.parentFile)
        assertEquals("gsr_data.csv", shimmerFile.name)
        val unknownFile = sessionDirectoryManager.getStandardFilePath(sessionDir, "unknown", "test.txt")
