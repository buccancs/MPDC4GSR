...
class FileTransferProtocol(
     ...
     */
    private suspend fun initializeTransfer(session: TransferSession): Unit {
     ...
     */
    private suspend fun transferFileInChunks(session: TransferSession): Unit = withContext(Dispatchers.IO) {
     ...
     */
    private suspend fun sendFileChunk(session: TransferSession, chunkIndex: Int, data: ByteArray) {
    ...
    private suspend fun checkResumeCapability(transferId: String): Long {
        val resumeQuery = JSONObject().apply {
     ...
     */
    private suspend fun handleTransferError(session: TransferSession, error: Exception) {
