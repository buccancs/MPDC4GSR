...
/**
 * Specialized thermal imaging component providing DeviceIrcmdControlManager functionality for the IRCamera system.
 *
...
Command交互class
    ...
    /**
     * Executes deviceircmdcontrolmanager operation with thermal imaging domain optimization.
     *
     */
...
getFPGAalgorithmparameter读取 PASS
...
将int数值conversion为占四个字节的bytearray，本method适用于(高位在前，低位在后)的顺序。  和bytesToInt2（）配套使用
    ...
    /**
大端modeconversion
bytearray中取int数值，本method适用于(低位在后，高位在前)的顺序。和intToBytes2（）配套使用
     */
...
读取出来的aint，共四个字节的值
    ...
    /**
inttype的bytearray，转为二进制字符串，然后根据传入的值做替换，最终输出拼装好的值
     *
     * @param name
     * @param ispParamReadByteArray
     * @param byteWidth
...
需要根据begin和end来截取要传入的值,然后替换读取出来的值
    ...
    /**
setISPalgorithm需要modify的filepath
会在重新回到预览页area的时候调用sendISPParammethod来setparameter
     *
     * @param param_path
     */
...
会在重新回到预览页area的时候调用sendISPParammethod来setparameter
     * <p>
ISPparameterset后，停图后不会saveparameter，firmware会重新下发
     *
...
然后单独modifybegin到end之前的值，重新写入进去
