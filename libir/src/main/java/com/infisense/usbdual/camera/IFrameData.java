...
public class IFrameData {
    ...
    /**
     * 缩放temperaturedata长度，YUV-422，故值为：
    ...
    /**
     * 缩放visible lightdata长度，ARGB，故值为：
     * 原始visible light宽度 x 原始visible light高度 x 4.
     */
    public static int P_IN_P_LEN = Const.DUAL_WIDTH * Const.DUAL_HEIGHT * 4;
    /**
     * 一帧除picture-in-picture缩放缩放visible lightdata之外的所有data长度，
     * 包含 fusionimage、原始infrared、原始temperature、缩放temperature、原始visible light、picture-in-picture缩放visible light data.
     * 值为上述data长度之和.
     */
    ...
    /**
     * 将指定帧data中 YUV-422 <b>缩放temperaturedata</b> copy到指定array中.
     */
    public static byte[] readRemapTempData(@NonNull byte[] frame, @Nullable byte[] remapTempData) {
        ...
        System.arraycopy(frame, FUSION_LEN + ORIGINAL_LEN + ORIGINAL_LEN, remapTempData, 0, remapTempData.length); //缩放temperaturedata，YUV-422
