...
class LibMatrixModuleTest {
    ...
    fun testMatrixTransposition() =
        runTest {
            ...
            assertEquals(
                "Transposed[0][0] should equal original[0][0]",
                originalMatrix[0][0],
                transposedMatrix[0][0],
                0.001f
            )
            assertEquals(
                "Transposed[1][0] should equal original[0][1]",
                originalMatrix[0][1],
                transposedMatrix[1][0],
                0.001f
            )
            assertEquals(
                "Transposed[2][1] should equal original[1][2]",
                originalMatrix[1][2],
                transposedMatrix[2][1],
                0.001f
            )
        }
    @Test
    ...
    fun testVectorOperations() =
        runTest {
            ...
            assertEquals(
                "Vector A magnitude should be sqrt(14)",
                kotlin.math.sqrt(14.0).toFloat(),
                magnitudeA,
                0.001f
            )
        }
    ...
    fun testMatrixDeterminant() =
        runTest {
            ...
            val determinant2x2 =
                matrix2x2[0][0] * matrix2x2[1][1] - matrix2x2[0][1] * matrix2x2[1][0]
        ...
        runTest {
            ...
            for (i in identityMatrix.indices) {
                for (j in identityMatrix[i].indices) {
                    if (i == j) {
                        assertEquals(
                            "Identity matrix diagonal should be 1.0",
                            1.0f,
                            identityMatrix[i][j],
                            0.001f
                        )
                        ...
                        assertEquals(
                            "Identity matrix off-diagonal should be 0.0",
                            0.0f,
                            identityMatrix[i][j],
                            0.001f
                        )
    ...
    fun testAsyncOperations() =
        runTest {
            ...
            assertEquals(
                "Async matrix operation should return correct value",
                context.packageName,
                result
            )
        }
