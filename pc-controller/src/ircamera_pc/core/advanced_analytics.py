...
class AdvancedAnalyticsEngine:
    ...
    def _extract_multimodal_features(self, session_key: str, timestamp: float) -> Optional[MultiModalFeatures]:
        ...
        try:
            ...
            facial_features = self._extract_facial_features(buffers.get(SensorModality.RGB_FACIAL, []))
            motion_features = self._extract_motion_features(buffers.get(SensorModality.MOTION, []))
            
            # Sensor fusion and unified metrics
            stress_level = self._fuse_stress_indicators(gsr_features, thermal_features, facial_features)
    ...
    def _extract_gsr_advanced_features(self, gsr_readings: List[SensorReading]) -> Dict[str, float]:
        ...
        try:
            ...
            features['rising_time_percent'] = float((np.sum(diff > 0) / len(diff)) * 100) if len(diff) > 0 else 0
            
            # Peak analysis with enhanced detection
            peaks, properties = signal.find_peaks(values, prominence=features['std'] * 0.3)
            features['peak_count'] = len(peaks)
            features['peak_frequency'] = len(peaks) / (len(values) / 60.0) if len(values) > 60 else 0  # per minute
            ...
            if len(peaks) > 0:
                ...
                features['peak_width_mean'] = float(np.mean(peak_widths)) if len(peak_widths) > 0 else 0
            else:
                features.update({
            ...
            if len(values) > 64:
                ...
                features['power_high'] = float(np.trapz(psd[high_band])) if np.any(high_band) else 0.0
                ...
                if total_power > 0:
                    ...
                    features['spectral_entropy'] = float(-np.sum(psd_norm * np.log2(psd_norm))) if len(psd_norm) > 0 else 0.0
    ...
    def _extract_thermal_features(self, thermal_readings: List[SensorReading]) -> Dict[str, float]:
        ...
        try:
            features = {}
            values = np.array([r.processed_value for r in thermal_readings[-180:]])  # Last 3 minutes
            ...
            features['temp_stability'] = 1.0 / (1.0 + features['std_temp']) if 'std_temp' in features else 0.0
    ...
    def _extract_facial_features(self, facial_readings: List[SensorReading]) -> Dict[str, float]:
        ...
        try:
            ...
            if facial_readings:
                ...
                features['expression_variability'] = float(np.std(values)) if len(values) > 1 else 0.0
    ...
    def _fuse_stress_indicators(self, gsr_features: Dict, thermal_features: Dict, facial_features: Dict) -> float:
    ...
    def _calculate_pattern_stability(self, session_key: str, current_pattern: BiometricPattern) -> float:
        ...
        same_pattern_count = sum(1 for f in recent_features if f.detected_pattern == current_pattern)
        
        return (same_pattern_count / len(recent_features)) * 100.0
    
    def _generate_behavioral_insights(self, session_key: str, features: MultiModalFeatures) -> List[BehavioralInsight]:
    ...
    def get_recent_insights(self, device_id: str, session_id: str, limit: int = 5) -> List[BehavioralInsight]:
    ...
    def generate_research_report(self, device_id: str, session_id: str, 
        ...
        try:
            ...
            effect_sizes = {
                "stress_mean_vs_baseline": (np.mean(stress_values) - 20) / np.std(stress_values) if np.std(stress_values) > 0 else 0,
                "arousal_variability": np.std(arousal_values) / np.mean(arousal_values) if np.mean(arousal_values) > 0 else 0
            }
            ...
            data_completeness = len([q for q in quality_scores if q > 80]) / len(quality_scores) * 100 if quality_scores else 0
            ...
            return ResearchMetrics(
                ...
                confidence_intervals={"stress_mean": (np.mean(stress_values) - 1.96*np.std(stress_values), 
    ...
    def export_research_data(self, device_id: str, session_id: str, 
        ...
        try:
            ...
            if format.lower() == "bids":
                ...
                physio_file = os.path.join(output_dir, f"sub-{device_id}_ses-{session_id}_physio.tsv")
                ...
                sidecar_file = os.path.join(output_dir, f"sub-{device_id}_ses-{session_id}_physio.json")
                with open(sidecar_file, 'w') as f:
                    json.dump(sidecar, f, indent=2)
                
                # Behavioral insights
                insights_data = [asdict(insight) for insight in insights]
                insights_file = os.path.join(output_dir, f"sub-{device_id}_ses-{session_id}_behavior.json")
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_feature_analyses": sum(len(history) for history in self.feature_history.values()),
            "total_insights_generated": sum(len(insights) for insights in self.insight_history.values()),
