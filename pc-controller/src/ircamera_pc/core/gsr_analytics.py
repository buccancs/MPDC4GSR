...
class GSRAnalytics:
    ...
    def __init__(self, 
        ...
        logger.info(f"GSR Analytics initialized: {window_size_seconds}s windows, {overlap_seconds}s overlap")

    def add_gsr_samples(self, device_id: str, session_id: str, 
                       gsr_values: List[float], timestamps: List[float]):
        """
        ...
        if len(self.device_buffers[device_key]) > max_samples:
            ...
            self.device_timestamps[device_key] = self.device_timestamps[device_key][excess:]
        ...
        if (time_since_last >= (self.window_size_seconds - self.overlap_seconds) and
            len(self.device_buffers[device_key]) >= self.window_size_samples):
            
            # Schedule analysis - handle both async and sync environments
            try:
                # Try async scheduling if event loop is running
                asyncio.create_task(self._analyze_window_async(device_id, session_id, current_time))
            except RuntimeError:
                # No event loop running, perform synchronous analysis
                self._analyze_window_sync(device_id, session_id, current_time)

    async def _analyze_window_async(self, device_id: str, session_id: str, current_time: float):
        ...
        try:
            ...
            await loop.run_in_executor(
                ...
                device_id, session_id, current_time
            )
        except Exception as e:
            logger.error(f"Error in async GSR analysis for {device_id}: {e}")

    def _analyze_window_sync(self, device_id: str, session_id: str, current_time: float):
        ...
        try:
            ...
            # Run analysis
            features = self._extract_features(device_id, session_id, window_data, window_timestamps)
            ...
            if features:
                ...
                if len(self.feature_history[device_key]) > 100:
                    self.feature_history[device_key] = self.feature_history[device_key][-100:]
                
                self.last_analysis[device_key] = current_time
                
                logger.debug(f"GSR analysis completed for {device_id}: stress={features.stress_score:.1f}")
                
        except Exception as e:
            logger.error(f"Error in sync GSR analysis for {device_id}: {e}")

    def _extract_features(self, device_id: str, session_id: str, 
                         gsr_data: np.ndarray, timestamps: np.ndarray) -> Optional[GSRFeatures]:
        """
        ...
        try:
            ...
            peak_frequency = (peak_count / len(gsr_clean)) * self.sampling_rate * 60  # per minute
            ...
            stress_score, stress_level, confidence = self._assess_stress(
                gsr_clean, mean_gsr, std_gsr, range_gsr, peak_frequency, rising_time, rapid_changes)
    ...
    def _calculate_rising_time(self, gsr_data: np.ndarray) -> float:
        ...
        try:
            ...
            return (rising_samples / total_samples) * 100.0 if total_samples > 0 else 0.0
        except:
            return 0.0
    ...
    def _analyze_frequency_domain(self, gsr_data: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            freqs, psd = signal.welch(gsr_data, fs=self.sampling_rate, nperseg=min(256, len(gsr_data)//4))
            ...
            power_low = float(np.trapz(psd[low_band], freqs[low_band])) if np.any(low_band) else 0.0
            power_mid = float(np.trapz(psd[mid_band], freqs[mid_band])) if np.any(mid_band) else 0.0
            power_high = float(np.trapz(psd[high_band], freqs[high_band])) if np.any(high_band) else 0.0
    ...
    def _assess_stress(self, gsr_data: np.ndarray, mean_gsr: float, std_gsr: float,
                      range_gsr: float, peak_frequency: float, rising_time: float,
                      rapid_changes: int) -> Tuple[float, StressLevel, float]:
        """
    ...
    def get_real_time_features(self, device_id: str, session_id: str) -> Optional[GSRFeatures]:
    ...
    def generate_session_report(self, device_id: str, session_id: str) -> Optional[GSRAnalysisReport]:
        """Generate comprehensive analysis report for a session"""
        device_key = f"{device_id}_{session_id}"
        
        if device_key not in self.feature_history or not self.feature_history[device_key]:
    ...
    def _generate_recommendations(self, average_stress: float, peak_stress: float,
                                stress_trend: str, stress_distribution: Dict[str, float]) -> List[str]:
        """Generate personalized recommendations based on stress analysis"""
        recommendations = []
        
        # High stress recommendations
        if average_stress > 70:
            recommendations.append("Consider stress reduction techniques such as deep breathing or meditation")
        ...
        if peak_stress > 85:
            ...
            recommendations.append("Consider professional stress management consultation")
            
        # Trend-based recommendations
        if stress_trend == "increasing":
            recommendations.append("Stress levels are increasing - consider intervention strategies")
            recommendations.append("Evaluate current stressors and coping mechanisms")
        elif stress_trend == "decreasing":
            recommendations.append("Stress levels are decreasing - maintain current strategies")
            
        # Distribution-based recommendations
        high_stress_time = stress_distribution.get("high", 0) + stress_distribution.get("very_high", 0)
        if high_stress_time > 30:
            recommendations.append(f"High stress detected {high_stress_time:.1f}% of the time")
            recommendations.append("Consider lifestyle modifications to reduce chronic stress")
    ...
    def export_features_csv(self, device_id: str, session_id: str, filename: str) -> bool:
    ...
    def get_stress_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            'active_sessions': len(self.feature_history),
            'sessions': {}
        }
