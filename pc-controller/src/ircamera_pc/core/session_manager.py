...
class EnhancedSessionManager:
    ...
    def add_state_callback(self, callback: Callable[[SessionState, Optional[SessionMetadata]], None]) -> None:
        """Add callback for session state changes."""
        self._state_callbacks.append(callback)
    
    def remove_state_callback(self, callback: Callable[[SessionState, Optional[SessionMetadata]], None]) -> None:
    ...
    def _notify_state_callbacks(self, state: SessionState, session: Optional[SessionMetadata] = None) -> None:
    ...
    async def stop_recording(self) -> bool:
        ...
        if self.current_session.started_at:
            duration = (self.current_session.stopped_at - self.current_session.started_at).total_seconds()
    ...
    def _update_session_state(self, new_state: SessionState) -> None:
        ...
        logger.info(f"Session {self.current_session.session_id} state: {old_state.value} -> {new_state.value}")
    ...
    def is_session_active(self) -> bool:
        ...
        return (self.current_session is not None and 
                self.current_session.state in [SessionState.ACTIVE, SessionState.RECORDING, SessionState.STOPPING])
