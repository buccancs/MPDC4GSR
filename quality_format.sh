...
format_files() {
    ...
    if command -v ktlint >/dev/null 2>&1; then
        local kotlin_files=$(find . -name "*.kt" -not -path "./build/*" | wc -l)
    ...
    if command -v prettier >/dev/null 2>&1; then
        local json_files=$(find . -name "*.json" -not -path "./build/*" -not -path "./node_modules/*" | wc -l)
...
analyze_quality() {
    ...
    if command -v ktlint >/dev/null 2>&1; then
        local kt_issues=$(ktlint 2>&1 | wc -l || echo "0")
