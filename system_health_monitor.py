...
class SystemHealthMonitor:
    ...
    def collect_system_metrics(self) -> SystemMetrics:
        ...
        # Gradle processes
        gradle_processes = len([p for p in psutil.process_iter(['name']) 
        ...
        if gradle_cache.exists():
            try:
                result = subprocess.run(['du', '-sm', str(gradle_cache)], 
        ...
        # Active validators
        active_validators = len([p for p in psutil.process_iter(['name']) 
                               if any(validator in p.info['name'].lower() 
    ...
    def _count_recent_errors(self) -> int:
        ...
        try:
            ...
            if gradle_log.exists() and time.time() - gradle_log.stat().st_mtime < 3600:  # 1 hour
    ...
    def print_dashboard(self, metrics: SystemMetrics, alerts: List[Dict]):
        ...
        print(f"   CPU Usage:     {metrics.cpu_percent:5.1f}% {'🔥' if metrics.cpu_percent > 80 else '✅'}")
        print(f"   Memory Usage:  {metrics.memory_percent:5.1f}% {'🔥' if metrics.memory_percent > 80 else '✅'}")
        print(f"   Disk Usage:    {metrics.disk_usage_percent:5.1f}% {'🔥' if metrics.disk_usage_percent > 90 else '✅'}")
        print()
        ...
        print(f"   Last Build Time:   {metrics.last_build_duration:.1f}s {'🐌' if metrics.last_build_duration > 300 else '⚡'}")
        print()
        
        # Code Quality
        print("📊 Code Quality:")
        print(f"   Quality Score:     {metrics.quality_score:.1f}/100 {'🌟' if metrics.quality_score > 80 else '⚠️' if metrics.quality_score > 60 else '🔥'}")
        print(f"   Compilation Errors: {metrics.compilation_errors} {'❌' if metrics.compilation_errors > 0 else '✅'}")
        print()
        ...
        if alerts:
            print("🚨 Active Alerts:")
            for alert in alerts:
                severity_icon = {'critical': '🔥', 'warning': '⚠️', 'info': '💡'}.get(alert['severity'], '📋')
    ...
    def _generate_recommendations(self, metrics: SystemMetrics) -> List[str]:
        ...
        if metrics.cpu_percent > 70:
            recommendations.append("Consider closing unnecessary applications to reduce CPU usage")
        ...
        if metrics.gradle_processes > 3:
            recommendations.append("Multiple gradle processes detected - consider using single daemon")
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            report = {
                ...
                "performance_analysis": {
                    ...
                    "build_performance_rating": "Excellent" if max_build_time < 120 else "Good" if max_build_time < 300 else "Needs Improvement",
                    "system_load_rating": "Low" if avg_cpu < 50 else "Medium" if avg_cpu < 75 else "High"
                },
                "alert_summary": {
    ...
    def _generate_health_recommendations(self, metrics: List, alerts: List) -> List[str]:
        ...
        if avg_cpu > 60:
            recommendations.append("System running hot - consider upgrading hardware or optimizing builds")
        
        if avg_memory > 70:
            recommendations.append("High memory usage detected - increase RAM or optimize IDE settings")
        
        if max_cache > 4000:
            recommendations.append("Large build cache detected - implement automated cache cleanup")
        
        if len([a for a in alerts if a[3] == 'critical']) > 0:
            recommendations.append("Critical alerts detected - immediate attention required")
        
        if len(alerts) > 20:
            recommendations.append("Frequent alerts - review system configuration and thresholds")
        
        return recommendations or ["System running optimally - no recommendations needed"]
...
def main():
    ...
    if args.report:
        ...
        # Save report to file
        report_file = Path(args.project_root) / f"health_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        print("IRCamera System Health Monitor")
        print("Use --dashboard for real-time monitoring or --report for health analysis")
