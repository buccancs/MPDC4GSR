...
from ircamera_pc.core.gsr_analytics import (
    GSRAnalytics, GSRFeatures, GSRAnalysisReport, StressLevel
)
...
class TestGSRAnalytics(unittest.TestCase):
    ...
    def generate_test_gsr_data(self, duration_seconds: int, pattern: str = "normal") -> tuple:
        ...
        if pattern == "normal":
            ...
            for i in range(5):
                ...
                if peak_idx < samples:
                    ...
                    peak_signal = 4.0 * np.exp(-((np.arange(end_idx - start_idx) - peak_width // 2) ** 2) / (peak_width / 4) ** 2)
            ...
            artifact_indices = np.random.choice(samples, size=samples // 50, replace=False)
            gsr_values[artifact_indices] += np.random.uniform(-10, 15, len(artifact_indices))
    ...
    def test_basic_feature_extraction(self):
        ...
        # Get features
        features = self.analytics.get_real_time_features(self.device_id, self.session_id)
        ...
        print(f"✓ Basic feature extraction: stress={features.stress_score:.1f}, level={features.stress_level.value}")
    ...
    def test_stress_detection(self):
        ...
        for pattern in test_patterns:
            ...
            if features:
                ...
                print(f"✓ {pattern.title()} pattern: stress={features.stress_score:.1f}, level={features.stress_level.value}")
        ...
        if "normal" in expected_stress_levels and "increasing" in expected_stress_levels:
            self.assertGreater(
    ...
    def test_session_report_generation(self):
        ...
        self.assertIn(report.stress_trend, ["increasing", "decreasing", "stable", "insufficient_data"])
        ...
        print(f"✓ Session report: {len(report.features)} features, avg_stress={report.average_stress_score:.1f}, trend={report.stress_trend}")
    ...
    def test_feature_export(self):
        ...
        expected_columns = [
            'timestamp', 'device_id', 'session_id', 'mean_gsr', 'std_gsr',
            'stress_score', 'stress_level', 'confidence'
        ]
    ...
    def test_artifact_handling(self):
        ...
        print(f"✓ Artifact handling: stress={features.stress_score:.1f}, confidence={features.confidence:.1f}")
    ...
    def test_performance_with_large_dataset(self):
        ...
        self.assertGreater(samples_per_second, 128)  # Should be faster than sampling rate
        
        print(f"✓ Performance test: {len(gsr_values)} samples processed in {processing_time:.2f}s "
...
class TestGSRReceiverAnalyticsIntegration(unittest.TestCase):
    ...
    async def test_integrated_analytics_workflow(self):
        ...
        analytics_data = self.gsr_receiver.get_real_time_analytics(device_id, session_id)
        ...
        if analytics_dir.exists():
            ...
            self.assertGreater(len(report_files), 0, "Analytics report should be generated")
            self.assertGreater(len(feature_files), 0, "Feature export should be generated")
...
def main():
    ...
    suite2 = unittest.TestLoader().loadTestsFromTestCase(TestGSRReceiverAnalyticsIntegration)
