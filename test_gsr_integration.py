...
from ircamera_pc.core.gsr_receiver import DeviceSession, GSRReceiver, GSRSample
...
class TestGSRHubSpokeIntegration(unittest.TestCase):
    ...
    async def test_gsr_receiver_initialization(self) -> Any:
        ...
        with sqlite3.connect(self.gsr_receiver.db_path) as conn:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='gsr_samples'"
            )
    ...
    async def test_data_export(self) -> Any:
        ...
        await self.gsr_receiver.end_session(self.test_device_id, self.test_session_id)
...
class TestNetworkServerGSRIntegration(unittest.TestCase):
    ...
    async def test_gsr_stream_registration(self) -> Any:
        ...
        # Handle registration
        response = await self.server._handle_gsr_stream_registration(
            message, mock_writer
        )
    ...
    async def test_gsr_data_stream_handling(self) -> Any:
        ...
        mock_writer = Mock()
        await self.server._handle_gsr_stream_registration(
            registration_message, mock_writer
        )
...
class TestGSRIntegrationEndToEnd(unittest.TestCase):
    ...
    async def test_complete_gsr_workflow(self) -> Any:
        ...
        for batch in range(5):  # 5 batches of data
            ...
            # Simulate heartbeat
            await receiver.handle_heartbeat(
                device_id,
                session_id,
                {"timestamp": time.time(), "buffer_size": 20 - batch * 4},
            )

        # Step 3: Send quality metrics
        await receiver.handle_quality_metrics(
            device_id,
            session_id,
            {
                "samples_sent": 50,
    ...
    def test_android_kotlin_integration_stubs(self) -> Any:
        ...
        gsr_streamer_path = (
            ...
            / "sensors"
            / "gsr"
            / "GSRNetworkStreamer.kt"
        )
        self.assertTrue(
            gsr_streamer_path.exists(), "GSRNetworkStreamer.kt should exist"
        )
